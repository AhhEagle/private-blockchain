...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      block.previousBlockHash =
        self.chain.length > 0 ? self.chain[self.chain.length - 1].hash : null;
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      resolve(self.chain.filter(element => element.hash === hash)[0]);
    });
  }
  ...
  getStarsByWalletAddress(address) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let personalBlocks = self.chain.filter(
        element => element.owner === address
      );
      ...
      stars = personalBlocks.map(element =>
         JSON.parse(hex2ascii(element.body))
      );
      console.log(stars);
